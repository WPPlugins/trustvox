...
class trustvox_settings_page {
	...
	public function trustvox_options() {
		...
		echo '<div class="wrap">';
			echo '<span style="float: right"><a href="http://site.trustvox.com.br/" target="_blank"><img src="' . plugins_url( '../assets/trustvox-logo.png', __FILE__ ) . '"></a></span></span>';
	...
	public function trustvox_settings() {
		...
		public function trustvox_settings_info() {
			...
			printf('<p>%s <a href="http://site.trustvox.com.br/contato/" target="_blank">%s</a>.</p>', $message, $click);
		}
	...
	public function trustvox_certificate() {
		...
		public function trustvox_certificate_info() {
			print __('Informações adicionais.', 'trustvox');
		}

		// Adiciona 
		private function trustvox_certificate_info_extra() {
			print '<h3>' . __('Opcional:', 'trustvox') . '</h3>';
			printf('<p>%s <code>%s</code></p>', __('Personalize o selo, inclua o shortcode em seu tema, ou via widget, páginas e posts com o código:', 'trustvox'), esc_html('[trustvox_selo_site_sincero]') );
			printf('<p>%s <code>%s</code></p>', __('Para aplicar direto no código-fonte, utilize o código:', 'trustvox'), esc_html("<?php echo do_shortcode('[trustvox_selo_site_sincero]'); ?>") );
		}
	...
	private function trustvox_faq() {
		print '<dl class="trustvox-faq">';
			print '<dt>' . __('O TrustVox é gratuito?') . '</dt>';
			printf('<dd>%s <a href="http://site.trustvox.com.br/contato/" target="_blank">%s</a>.</p>', __('Não. É necessário adquirir um plano.'), __('Comece agora') );
			...
			print '<dt>' . __('Como funciona o TrustVox?') . '</dt>';
			printf('<dd>%s <a href="http://site.trustvox.com.br/" target="_blank">%s</a>.</p>', __('Somos um certificador de reviews. Mais confiança, mais vendas!'), __('Saiba mais') );

			print '<dt>' . __('Os comentários da minha loja vão ser desabilitados?') . '</dt>' .
			...
			print '<dt>' . __('É possível migrar/importar as opiniões que já tenho em minha loja para o TrustVox?') . '</dt>' .
			...
			print '<dt>' . __('Preciso instalar o HTML da Trustvox ?') . '</dt>' . 
			      '<dd>' . __('Não. A instalação é totalmente plug-and-play. A Trustvox irá aparecer automaticamente no detalhe de todas as suas páginas de produtos.') . '</dt>';
			...
			print '<dt>' . __('E as vendas/pedidos antigas(os)?') . '</dt>' .
			      '<dd>' . __('Será feita uma importação automática de todos os pedidos dos últimos 90 dias com o status "Concluído" a partir da ativação do TrustVox em sua loja.') . '</dt>';

			print '<dt>' . __('Como posso incluir o Selo Site Sincero em minha loja?') . '</dt>' .
			      '<dd>' . __('Se a sua loja já possui mais de 30 dias e no mínimo 50 opiniões publicadas na Trustvox, então ela já está qualificada para ter o Selo Site Sincero.') . '</dt>' .
			      '<dd>' . __('É bem simples, é só entrar em contato com a Trustvox em suporte@trustvox.com.br para gerar o seu Certificado de Sinceridade.') . '</dt>' .
			      '<dd>' . __('Assim que obtiver o código, é possível adicioná-lo na aba "Selo Site Sincero", escolher o modo de exibição e pronto.') . '</dt>';

			print '<dt>' . __('Quer saber mais?') . '</dt>';
			printf('<dd>%s <a href="http://help.trustvox.com.br/" target="_blank">http://help.trustvox.com.br/</a></p>', __('Acesse nossa seção Help no endereço: ') );
			...
			<div class="trustvox-credits">
				<div class="trustvox-credits__item">
					<div class="trustvox-credits__thumb">
						<a href="http://site.trustvox.com.br/?&utm_source=pluginTrustvox&utm_medium=link&utm_campaign=' . get_site_url() . '" target="_blank"><img src="' . plugins_url( '../assets/credits-logo-trustvox.jpg', __FILE__ ) . '"></a>
					</div>
					<div class="trustvox-credits__infos">
						<p class="trustvox-credits__title"><a href="http://site.trustvox.com.br/?&utm_source=pluginTrustvox&utm_medium=link&utm_campaign=' . get_site_url() . '" target="_blank">TrustVox - Sinceridade não tem preço</a></p>
						<ul>
							<li><a href="https://www.facebook.com/trustvox" target="_blank"><img src="' . plugins_url( '../assets/social-facebook.jpg', __FILE__ ) . '"></a></li>
							<li><a href="http://www.twitter.com/trustvox" target="_blank"><img src="' . plugins_url( '../assets/social-twitter.jpg', __FILE__ ) . '"></a></li>
						</ul>
					</div>
				</div>
				<div class="trustvox-credits__item">
					<div class="trustvox-credits__thumb">
						<a href="http://lampejos.com.br/?&utm_source=pluginTrustvox&utm_medium=link&utm_campaign=' . get_site_url() . '" target="_blank"><img src="' . plugins_url( '../assets/credits-logo-lampejos.jpg', __FILE__ ) . '"></a>
					</div>
					<div class="trustvox-credits__infos">
						<p class="trustvox-credits__title"><a href="http://lampejos.com.br/?&utm_source=pluginTrustvox&utm_medium=link&utm_campaign=' . get_site_url() . '" target="_blank">Lampejos</a></p>
						<ul>
