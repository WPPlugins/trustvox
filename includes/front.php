	...
	if ( is_singular('product') ) {
		...
		print '
				(function() {
					...
					tv.charset = "UTF-8";
					tv.src = \'//static.trustvox.com.br/assets/trustvox.1.min.js\';
...
function trustvox_template_single_reviews() {
	...
	print '<div id="_trustvox_widget_container" class="trustvox-widget-container" style="margin-bottom: 30px;"><h2 class="trustvox-widget-title">' . $trustvox_options_widget_title . '</h2><div id="_trustvox_widget"></div></div>';
}
...
function trustvox_scripts_reviews_stars() { 
	if ( !is_admin() ) {
		...
		print '
			...
			<script type="text/javascript" async="true" src="' . esc_url('//s3-sa-east-1.amazonaws.com/trustvox-rate-widget-js/widget.js') . '"></script>';
	...
	if ( ! is_admin() ) {
		...
		if ( isset($trustvox_certificate['code']) and isset($trustvox_certificate['position']) and $trustvox_certificate['position'] != 'none' ) {
			print '
				...
				<link rel="stylesheet" type="text/css" href="https://s3-sa-east-1.amazonaws.com/trustvox-certificate/' . $trustvox_certificate['position'] . '/style-selo.css" />
				<script type="text/javascript">
					var _trustvox_certificate = _trustvox_certificate || [];
					    _trustvox_certificate.push([\'_certificateId\', ' . wp_json_encode($trustvox_certificate['code']) . ' ]) ;
					(function() {
						var tv = document.createElement(\'script\'); tv.type = \'text/javascript\'; tv.async = true;
						tv.src = \'' . esc_url('//s3-sa-east-1.amazonaws.com/trustvox-certificate-modal-js/widget.js') . '\';
						var s = document.getElementsByTagName(\'script\')[0]; s.parentNode.insertBefore(tv, s);
					})();
				</script>
			';
...
function trustvox_shortcode_selo_site_sincero(){
	...
	if ( isset($trustvox_certificate['code']) ) {
		return '
			<div id="trustvox-selo-site-sincero-shortcode" class="ts-modal-trigger" style="cursor:pointer;">
...
class trustvox_widget_selo_site_sincero extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		$trustvox_certificate = get_option('trustvox_certificate', false);
		print '
			<div id="trustvox-selo-site-sincero-widget" class="ts-modal-trigger" style="cursor:pointer;">
				<img src="' .plugins_url( '../assets/selo-site-sincero.png', __FILE__ ) . '" />
			</div>
			<script type="text/javascript">
				var _trustvox_certificate = _trustvox_certificate || [];
				    _trustvox_certificate.push([\'_certificateId\', ' . wp_json_encode($trustvox_certificate['code']) . ' ]) ;
